    ...
    const attachButton = (answerBox) => {
        ...
        fileInput.addEventListener('change', async () => {
            ...
            try {
                ...
                updateResultPanel(resultPanel, result.raw_latex || '', stackResult, answerBox);
        ...
        mobileBtn.addEventListener('click', async () => {
            try {
                ...
                mobilePanel._status.textContent = 'Waiting for mobile upload...';
                    ...
                    try {
                        ...
                        if (result.ready) {
                            ...
                            if (resultVersion !== lastMobileResultVersion) {
                                ...
                                mobilePanel._status.textContent = 'Successfully received mobile result. You can upload another photo with the same QR code.';
                                updateResultPanel(resultPanel, result.raw_latex || '', stackResult, answerBox);
                                setAnswerValue(answerBox, stackResult);
                            }
                            ...
                            window.clearInterval(mobilePollTimer);
                            mobilePollTimer = null;
                            mobilePanel._status.textContent = 'This mobile upload session has expired.';
                        } else {
                            mobilePanel._status.textContent = 'Waiting for mobile upload...';
                        }
                        ...
                        if (Date.now() - start > timeoutMs) {
                            window.clearInterval(mobilePollTimer);
                            mobilePollTimer = null;
                            mobilePanel._status.textContent = 'Timeout waiting for result. Please create a new session.';
                        }
                    } catch (error) {
                ...
                window.alert('Failed to create mobile session: ' + error.message);
                mobileBtn.disabled = false;
                mobileBtn.textContent = config.mobilebtn || 'Mobile Math Upload';
            }
        });
